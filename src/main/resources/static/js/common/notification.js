...
export const notificationToggle = (btn) => {
    const dropdown = btn.parentElement?.querySelector('.btn-notification');
    if (!dropdown)
        return;
...
export const loadNotificationTab = async (type) => {
    const container = document.querySelector('.btn-notification .notification-list');
    const header = document.querySelector('.noti-header');
...
const updateTabUI = (type) => {
    const tabs = document.querySelectorAll('.btn-notification .btn-tab');
    tabs.forEach(tab => {
...
export const handleNotificationClick = async (e, notificationId, url, isRead) => {
...
export const openUserNoticeModal = async () => {
