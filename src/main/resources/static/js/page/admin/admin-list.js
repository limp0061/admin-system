...
document.addEventListener('keydown', (e) => {
    ...
    if (isModalInput) {
        ...
        if (e.key === 'Enter') {
            e.preventDefault();
            const firstItem = document.querySelector('#common-select-ul .select-li');
...
const searchInput = async (input, keyword) => {
    const users = await sendRequest(`/api/v1/users/search?keyword=${encodeURIComponent(keyword)}`, { method: 'GET' });
    const commonUl = document.getElementById('common-select-ul');
    let html;
...
const openAdminModal = async (mode, btn) => {
...
const saveAdmin = async () => {
    ...
    if (!roleId)
        return showToast("권한을 선택해주세요.", "error");
    const requestData = {
        id: Number(userId),
