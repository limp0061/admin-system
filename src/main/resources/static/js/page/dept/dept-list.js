 ...
 * 부서 구성원 변경
 */
const openDeptUserModal = async (btn) => {
...
const userRemoveDept = async (btn) => {
    const userInfo = document.querySelectorAll('.dept-user-info');
...
const searchModal = () => {
    const modalContainer = document.getElementById("modal-form");
    const deptId = modalContainer.querySelector('#select-dept-id')?.value;
    const keyword = modalContainer.querySelector('input[name="keyword"]')?.value;
...
const moveUser = () => {
    ...
    if (!wrapper)
        return;
    const checkedBoxes = wrapper.querySelectorAll(".check-box:checked");
    if (checkedBoxes.length == 0)
        return;
    ...
    checkedBoxes.forEach(box => {
        ...
        if (targetUl.querySelector(`li[data-id="${id}"]`))
            return;
        const liHtml = `
...
const removeUser = () => {
    ...
    const checkedBoxes = ul.querySelectorAll('.check-box:checked');
    if (checkedBoxes.length == 0)
        return;
    checkedBoxes.forEach(box => {
