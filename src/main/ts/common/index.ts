...
const exposeToWindow = () => {
  ...
  w.goToNextPage = (current: number, total: number) =>
    PaginationUtils.goToNextPage(current, total);
  w.changeSize = (size: number) => PaginationUtils.changeSize(size);
  w.filterStatus = (status: string | null) =>
    PaginationUtils.filterStatus(status);
...
const initEventListeners = () => {
  ...
  document.addEventListener('click', (event: MouseEvent) => {
    ...
    if (
      notificationBox &&
      !notificationBox.contains(target) &&
      toggleBtn &&
