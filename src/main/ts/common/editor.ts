...
export class EditorManager {
   ...
   */
  async init(selector: string, uploadUrl: string): Promise<void> {
    if (this.instance) return; // 이미 있으면 중복 생성 방지

    try {
      this.instance = await ClassicEditor.create(
        document.querySelector(selector),
        editorConfig,
      );
      this.instance.plugins.get('FileRepository').createUploadAdapter = (
        loader: any,
      ) => {
