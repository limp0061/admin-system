...
export const loadNotificationTab = async (type: 'ALARM' | 'NOTICE') => {
  const container = document.querySelector(
    '.btn-notification .notification-list',
  );
  ...
  try {
                   ...
                   <div class="${type == 'ALARM' ? 'notification-row' : ''} flex flex-col gap-1.5 p-4 border-b border-gray-100 transition-all cursor-pointer 
                        ${
                          item.isRead
                            ...
                            : 'bg-white hover:bg-blue-50/30 dark:bg-[#2a2a2a] dark:hover:bg-zinc-800/30'
                        }"
                            onclick="handleNotificationClick(event, ${item.notificationId}, '${item.url}', ${item.isRead})">
                        <div class="flex justify-between items-center">
                            <span class="notification-date text-[10px] font-medium ${item.isRead ? 'text-gray-300 dark:text-gray-600' : 'text-blue-500 dark:text-blue-400'}">
                                ${formatDiffDate(item.createdAt)}
                            </span>
                        </div>
                        <div class="notification-title text-[13px] leading-relaxed break-keep ${item.isRead ? 'text-gray-400 dark:text-gray-600' : 'text-gray-800 font-semibold dark:text-gray-100'}">
                            ${item.title}
                        </div>
                   </div>`,
        )
    ...
    container.innerHTML =
      '<div class="p-4 text-center text-xs text-red-400">데이터를 불러오지 못했습니다.</div>';
  }
};

const updateTabUI = (type: 'ALARM' | 'NOTICE') => {
  const tabs = document.querySelectorAll<HTMLButtonElement>(
    '.btn-notification .btn-tab',
  );
  tabs.forEach((tab) => {
...
export const handleNotificationClick = async (
  ...
  try {
    ...
    const requestUrl =
      notificationId != null
        ? `${url}${separator}notificationId=${notificationId}`
        : url;
    ...
    if (root) {
      ...
      const count = await sendRequest('/api/v1/notifications/unread-count', {
        method: 'GET',
      });
