...
const selectData = (btn: HTMLElement) => {
  ...
  const btnDropDown = container.querySelector(
    'button[data-action="btnDropDown"] span',
  );
  if (btnDropDown) {
...
const search = async (): Promise<void> => {
  const auditRequest: AuditRequest = {
    action:
      (document.getElementById('action') as HTMLInputElement).value || null,
    targetEntity:
      (document.getElementById('targetEntity') as HTMLInputElement).value ||
      null,
    startAt:
