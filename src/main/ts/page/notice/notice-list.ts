...
const searchFilter = (): void => {
  const filter =
    (document.getElementById('noticeType') as HTMLInputElement)?.value || 'ALL';
  const type =
    (document.getElementById('tabType') as HTMLInputElement)?.value || 'ALL';
...
const openNoticeModal = async (
  ...
  try {
    ...
    if (root) {
      ...
      await noticeEditor.init(
        '#editor',
        '/api/v1/files/editor-upload?type=NOTICE',
      );

      const startVal = (document.getElementById('startAt') as HTMLInputElement)
        .value;
      const endVal = (document.getElementById('endAt') as HTMLInputElement)
        .value;
      ...
      const realTimeInput = document.getElementById(
        'isRealTimeNoticed',
      ) as HTMLInputElement;
...
const saveNotice = async (): Promise<void> => {
  ...
  const realTimeCheckbox = document.getElementById(
    'isRealTimeNoticed',
  ) as HTMLInputElement;
  ...
  const requiredFields: Record<
    string,
    { label: string; type?: 'input' | 'select' }
  > = {
    type: { label: '유형' },
    title: { label: '제목' },
    content: { label: '내용' },
