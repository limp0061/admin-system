...
document.addEventListener('keydown', (e: KeyboardEvent) => {
  ...
  const isModalInput =
    target.tagName === 'INPUT' && target.closest('#admin-form');
  if (isModalInput) {
    if (e.key === 'Escape') {
      e.preventDefault();
  ...
  if (!users || users.length === 0) {
    ...
    html = users
      .map(
        ...
        <li class="select-li p-3 hover:bg-slate-50 cursor-pointer border-b last:border-0 dark:border-zinc-700" 
            ...
            <div class="text-xs text-gray-500 py-2 dark:text-slate-400">${user.deptName} | ${user.emailId}</div>
        </li>
    `,
      )
      .join('');
...
const addIps = () => {
  ...
  const currentIps = Array.from(ipListContainer.querySelectorAll('.ip')).map(
    (el) => el.textContent,
  );
...
const saveAdmin = async (): Promise<void> => {
  ...
  if (!userId)
    return showToast('관리자로 등록할 유저를 선택해주세요.', 'error');
