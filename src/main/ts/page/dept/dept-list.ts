...
const expandAll = (): void => {
  document
    .querySelectorAll('ul[data-parent]')
    .forEach((ul) => ul.classList.remove('hidden'));

  document
    .querySelectorAll('[data-action="toggle"] svg')
...
const openDeptUserConfirmModal = async (
  mode: 'EDIT' | 'DEL',
): Promise<void> => {
  const checkedBoxes =
    document.querySelectorAll<HTMLInputElement>('.check-box:checked');
...
const userChangeDept = async (btn: HTMLElement): Promise<void> => {
  const selectedUsers = document
    .getElementById('selected-user-list')
    ?.querySelectorAll('li');
  ...
  const targetDeptInput = document.getElementById(
    'targetDeptId',
  ) as HTMLInputElement;
...
const searchModal = (): void => {
  const modalContainer = document.getElementById('modal-form');
  const deptId = (
    modalContainer.querySelector('#select-dept-id') as HTMLInputElement
  )?.value;
  const keyword = (
    modalContainer.querySelector('input[name="keyword"]') as HTMLInputElement
  )?.value;
...
const moveUser = () => {
  ...
  const checkedBoxes =
    wrapper.querySelectorAll<HTMLInputElement>('.check-box:checked');
  ...
  checkedBoxes.forEach((box) => {
    ...
    const liHtml = `
            <li class="flex items-center justify-between p-2 border-b bg-white text-xs hover:cursor-pointer hover:bg-gray-50 dark:bg-[#1e1e1e] dark:text-slate-300 dark:border-zinc-700 dark:hover:bg-zinc-800/30" 
  ...
  const allBoxes = wrapper.querySelectorAll<HTMLInputElement>(
    'input[type="checkbox"]',
  );
  allBoxes.forEach((cb) => {
    cb.checked = false;
...
const removeUser = () => {
  ...
  const checkedBoxes =
    ul.querySelectorAll<HTMLInputElement>('.check-box:checked');
  if (checkedBoxes.length == 0) return;
